...
def _learn_node_parameter_w(outputs, inputs=None):
    ...
    """ YOUR CODE HERE """
    n = outputs.shape[0]
    inputs = np.ones(n) if num_inputs == 0 else np.c_[np.ones(n), inputs]

    lhs = inputs.T @ inputs
    lhs = lhs.reshape(1, 1) if lhs.ndim == 0 else lhs

    rhs = inputs.T @ outputs
    rhs = rhs.reshape(1, 1) if rhs.ndim == 0 else rhs

    weights = np.linalg.solve(lhs, rhs)
    weights = weights.reshape(1, 1) if weights.ndim == 0 else weights
    """ END YOUR CODE HERE """
...
def _learn_node_parameter_var(outputs, weights, inputs):
    ...
    inputs = inputs.reshape(-1, 1) if inputs.ndim == 1 else inputs
    outputs = outputs.reshape(-1, 1)
    weights = weights.reshape(-1, 1)

    arg = outputs - (inputs @ weights)
    var = (arg.T @ arg).item() / n
    """ END YOUR CODE HERE """
...
def _get_learned_parameters(nodes, edges, observations):
    ...
    """ YOUR CODE HERE """
    graph = nx.DiGraph(list(map(tuple, edges)))
    ...
    for node in nodes:
        ...
        if parents:
            inputs = np.transpose(np.array([observations[p] for p in parents]))
        weights = _learn_node_parameter_w(outputs, inputs)
        var = _learn_node_parameter_var(outputs, weights, inputs)

        params = {"variance": var}
        for i, w in enumerate(weights):
            params["bias" if i == 0 else str(i - 1)] = w

        parameters[str(node)] = params
